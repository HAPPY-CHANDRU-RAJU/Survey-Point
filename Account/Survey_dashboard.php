...
$redsqu = "UPDATE `Survey_Activty` SET `Survey_Act_Status` = 'ACTIVE' WHERE `Survey_Act_Id` = '$Actvid'";
				}
$redsfs = $conn->prepare($redsqu);
$redsfs->execute();

								$errTyp = "success";
								$errMSG = "UPDATED SURVEY SUCCESSFULLY !!!";
...
<head>
	...
	<title> Survey Dashboard - Survey Point</title>
	<script>
		function myCopy(){
		...
		<div class="row">
			...
			<div class="col-sm-10" style="overflow: scroll;height: 500px;">
				<table class="table table-responsive">
					...
					$qs1 = "SELECT * FROM `Survey_Activty` WHERE `Survey_Act_Author_Id`='$emid'  ORDER BY `Survey_Act_Cate_name`  AND `Survey_Act_DOC` ASC ";
					...
					while($row2 = $rs23->fetch()){
						...
						<td>'.date('M j Y',strtotime($row2["Survey_Act_DOC"])).'</td>'; 
								...
								if($row2["Survey_Act_Status"]=="ACTIVE"){
									$display = "INACTIVE";
									$cole = "danger";
									$sta = "delete";
								}else{
						...
						<td><a href="answers.php?source_um=accept&action=view&Activity_Id=<?php echo $row2["Survey_Act_Id"]; ?>"><button class="btn btn-sm btn-primary" >
									VIEW
						</button></a></td>
						<td><button class="btn btn-sm btn-<?php echo $cole; ?>" onclick="location.href='?source_um=accept&action=<?php echo $sta; ?>&Activity_Id=<?php echo $row2["Survey_Act_Id"]; ?>'">
									<?php echo $display; ?>
						</button></td>
					<?php
					echo '</tr>';
